...
class Model():
	...
	def create_model(self):
		# 'seq_len' means question sequences	batch_size32,seq_len200，占位符创建tf张量32*200
		...
		self.target = tf.compat.v1.placeholder(tf.float32, [self.args.batch_size, self.args.seq_len], name='target')

		# Initialize Memory  初始化记忆阵20*50,20*200 key和value
		with tf.variable_scope('Memory'):	#scope用法，是变量创建的一种用法
			init_memory_key = tf.get_variable('key', [self.args.memory_size, self.args.memory_key_state_dim], \
				initializer=tf.truncated_normal_initializer(stddev=0.1))
			###k值和value矩阵初始化，truncated_normal_initializer函数从截断的正态分布中输出随机值
				##get_variable(name，shape，dtype，initializer,trainable)，name和shape为必要参数
				 #scpoe为作用域
		...
		# tf.tile(inputs, multiples) : multiples length must be thes saame as the number of dimensions in input数据扩展
		# tf.stack takes a list and convert each element to a tensor
		#tile()函数进行张量扩展，输出维度不变,expand_dims(输入，axis轴)为维度扩展,三种模式选轴，二维情况下为例，0即加入1，1即在中间加入1，2即最后加入1，变为三维张量
		#stack即将两个张量拼接为高阶张量
		init_memory_value = tf.tile(tf.expand_dims(init_memory_value, 0), tf.stack([self.args.batch_size, 1, 1]))
		print(init_memory_value.get_shape())	#assist2009__updated数据集下得出为32*20*200的张量

		#构建memory内容，建立DKVMN对象20，50，200，20*50，32*20*200
		...
		# Embedding to [batch size, seq_len, memory_state_dim(d_k or d_v)]嵌入200，200，N
		with tf.variable_scope('Embedding'):
			# A 初始化两个矩阵111*50，221*200，q与qa
		...
		# print('q_data shape is%s'%self.q_data.get_shape()) 32*200
		# Embedding to [batch size, seq_len, memory key state dim]32*200*50
		# print('q_embed_mtx shape is%s'%q_embed_mtx.get_shape())
		q_embed_data = tf.nn.embedding_lookup(q_embed_mtx, self.q_data)		#embedding_lookup函数，按给出的数据显示张量。所显示的张量合在一起组成新的张量。给出数据为张量时候，根据张量维度进行扩展。
		# List of [batch size, 1, memory key state dim] with 'seq_len' elements
		# print('Q_embedding shape : %s' % q_embed_data.get_shape())		#32*200*50
		#split函数进行切分，按第二维切分，切分个数为200
		...
		for i in xrange(self.args.seq_len):
			...
			q = tf.squeeze(slice_q_embed_data[i], 1) #删除维度为1的维，降维处理，q即32*50
			# print(q.get_shape())
			# Attention, [batch size, memory size]
			self.correlation_weight = self.memory.attention(q)	#相关权重计算 得出32*20
			# Read process, [batch size, memory value state dim] 读过程，
...
#		grad_clip = [(tf.clip_by_value(grad, -self.args.maxgradnorm, self.args.maxgradnorm), var) for grad, var in grads]
	...
	def train(self, train_q_data, train_qa_data, valid_q_data, valid_qa_data):
		...
		shuffle_index = np.random.permutation(train_q_data.shape[0])	#np.random.permutation对数组行数随机排序后输出numpy数组类型，numpy数组中shape函数0，1表示输出行或者列的规模
		q_data_shuffled = train_q_data[shuffle_index]	#数据按行乱序后保存为新的numpy数组
		qa_data_shuffled = train_qa_data[shuffle_index]

		training_step = train_q_data.shape[0] // self.args.batch_size	#数据行数除以batch_size，这里是740//32
		...
		for epoch in xrange(0, self.args.num_epochs):
			...
			for steps in xrange(training_step):
				# [batch size, seq_len]
				q_batch_seq = q_data_shuffled[steps*self.args.batch_size:(steps+1)*self.args.batch_size, :]		#数组32*200
			...
			all_accuracy.append(self.accuracy)
